...
import {
  ...
  ChevronDown, Flame, ClipboardCheck, Users, LogOut, Shield, RefreshCw, 
  HelpCircle, MessageSquare, AlertTriangle, Monitor, Sliders, ExternalLink, X
} from 'lucide-react';
        ...
        {animations.map((anim) => (
          ...
          >
            ...
            >
              ...
              <Flame size={80} className="text-[#FBBF24] relative z-10 drop-shadow-[0_0_15px_rgba(249,115,22,1)]" fill="#F97316" />
            </motion.div>
            <motion.span 
...
export default function Layout() {
  ...
  const navLinks = [
    { n: 'Dashboard', p: '/' }, { n: 'Missions', p: '/missions' }, { n: 'Learning Hub', p: '/learn' },
    { n: 'Analytics', p: '/analytics' }, { n: 'Collaboration', p: '/nexus' }, { n: 'Approval Queue', p: '/submissions' },
    { n: 'Member List', p: '/students' }, { n: 'Leaderboard', p: '/rankings' }, { n: 'Reward Vault', p: '/badges' },
    { n: 'Career Path', p: '/career' }, { n: 'Impact Map', p: '/map' }, { n: 'Account Center', p: '/profile' },
    { n: 'System Alerts', p: '/alerts' }, { n: 'Help Desk', p: '/help' }, { n: 'EcoLearn Prime', p: '/pro' }
  ];
  
  const searchResults = searchQuery 
    ? navLinks.filter(l => l.n.toLowerCase().includes(searchQuery.toLowerCase()))
    : navLinks.slice(0, 5);
    ...
    <div className="flex h-screen overflow-hidden font-['Inter']" style={{ background: 'var(--bg-base)' }}>
      ...
      <aside className="relative z-10 w-[260px] h-full flex flex-col border-r overflow-y-auto no-scrollbar shrink-0"
        ...
        <div className="px-5 pt-5 pb-2 flex items-center gap-3">
          ...
          <span className="text-[15px] font-bold text-white tracking-tight">EcoLearn <span style={{ color: '#22D3EE' }}>Protocol</span></span>
        </div>
        ...
        <div className="px-4 pt-3 pb-1">
          <motion.div whileTap={{ scale: 0.98 }}
            ...
            style={{ background: profileOpen ? 'rgba(0,242,254,0.06)' : 'rgba(255,255,255,0.03)', border: profileOpen ? '1px solid rgba(0,242,254,0.15)' : '1px solid rgba(255,255,255,0.06)' }}>
          ...
          <AnimatePresence>
            {profileOpen && (
              ...
              >
                <div className="mt-1.5 py-1.5 px-1 rounded-xl space-y-0.5" style={{ background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)' }}>
                  ...
                  <Link to="/profile" onClick={() => { setProfileOpen(false); toast('Opening settings...', { icon: '⚙️' }); }}>
                    <div className="flex items-center gap-2.5 px-3 py-2 rounded-lg text-[11px] font-medium text-[#94A3B8] hover:text-white hover:bg-white/[0.04] cursor-pointer transition-all">
        ...
        <div className="px-4 mb-2">
          <motion.div whileHover={{ borderColor: 'rgba(0, 242, 254, 0.3)' }} onClick={() => setSearchOpen(true)}
      ...
      <main className="relative flex-1 min-w-0 h-full overflow-y-auto overflow-x-hidden z-10">
        ...
        <header className="h-14 flex items-center justify-between px-7 sticky top-0 z-40 backdrop-blur-xl"
          ...
          <div className="flex items-center gap-2.5">
                ...
                {openDropdown === 'help' && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    ...
                    <div className="p-4 border-b border-white/5 bg-gradient-to-r from-transparent to-white/5">
                      <h3 className="text-sm font-bold text-white flex items-center gap-2"><HelpCircle size={14} className="text-[#00F2FE]"/> Communications Channel</h3>
                      <p className="text-[11px] text-[#64748B] mt-1">Direct uplink to EcoLearn Support AI.</p>
                    </div>
                    <div className="p-2 space-y-1">
                      {[
                        { icon: MessageSquare, label: 'Live Comm-Link', desc: 'Chat with Support AI', to: '/help' },
                        { icon: BookOpen, label: 'Knowledge Base', desc: 'Browse mission protocols', to: '/help' },
                        { icon: AlertTriangle, label: 'Report Anomaly', desc: 'File a bug report', to: '/help' }
                      ].map((item, idx) => (
                        <Link to={item.to} key={idx} onClick={() => setOpenDropdown(null)}>
                          <div className="flex items-center gap-3 p-3 rounded-xl hover:bg-white/5 cursor-pointer transition-all group">
                            ...
                            <div>
                              <p className="text-[12px] font-bold text-white/90 group-hover:text-white">{item.label}</p>
                ...
                {openDropdown === 'alerts' && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    ...
                    <div className="p-4 border-b border-white/5 flex justify-between items-center bg-gradient-to-r from-transparent to-[#EF4444]/5">
                      <div>
                        <h3 className="text-sm font-bold text-white flex items-center gap-2">System Alerts <span className="text-[10px] font-bold bg-[#EF4444] text-white px-2 py-0.5 rounded-full">3 NEW</span></h3>
                      </div>
                      <button className="text-[10px] text-[#94A3B8] hover:text-white">Mark All Read</button>
                    </div>
                    <div className="max-h-80 overflow-y-auto no-scrollbar">
                      {[
                        { title: 'Mission Approved', desc: 'Your "City Biodiversity" proof was accepted. +500 XP.', time: '2m ago', icon: Target, c: '#10B981' },
                        { title: 'Alliance Invite', desc: 'You have been invited to "Green Vanguard".', time: '1h ago', icon: Users, c: '#3B82F6' },
                        { title: 'System Warning', desc: 'Terminal connection latency spike detected.', time: '3h ago', icon: AlertTriangle, c: '#F59E0B' }
                      ].map((alert, idx) => (
                        <div key={idx} className="flex gap-3 p-4 border-b border-white/5 hover:bg-white/5 transition-colors cursor-pointer relative overflow-hidden group">
                          <div className="absolute left-0 top-0 bottom-0 w-1 opacity-0 group-hover:opacity-100 transition-opacity" style={{ background: alert.c }} />
                          <div className="w-8 h-8 shrink-0 rounded-full flex items-center justify-center mt-0.5" style={{ background: `${alert.c}15`, color: alert.c }}>
                            <alert.icon size={14} />
                          </div>
                          <div>
                            <p className="text-[12px] font-bold text-white">{alert.title}</p>
                            <p className="text-[11px] text-[#64748B] mt-0.5">{alert.desc}</p>
                            <p className="text-[9px] text-[#475569] mt-1 font-bold">{alert.time}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                    <div className="p-3 text-center border-t border-white/5">
                      <Link to="/alerts" onClick={() => setOpenDropdown(null)}>
                        <button className="text-[11px] font-bold text-[#00F2FE] hover:text-white transition-colors">View Alert Archive</button>
                      </Link>
                    </div>
                  </motion.div>
                )}
            ...
            {/* Settings Dropdown */}
            <div className="relative">
              <motion.button whileHover={{ scale: 1.05, rotate: 30 }} whileTap={{ scale: 0.95 }}
              ...
              <AnimatePresence>
                {openDropdown === 'settings' && (
                  <motion.div initial={{ opacity: 0, y: 10, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: 10, scale: 0.95 }}
                    ...
                    <div className="p-4 border-b border-white/5 bg-gradient-to-r from-transparent to-white/5">
                      <h3 className="text-sm font-bold text-white flex items-center gap-2"><Sliders size={14} className="text-[#94A3B8]"/> Quick Config</h3>
                    </div>
                    <div className="p-3 space-y-2">
                       <div className="flex justify-between items-center p-2 rounded-xl hover:bg-white/5 cursor-pointer" onClick={() => toast.success('Telemetry streaming active', {id:'st1'})}>
                         <span className="text-[12px] font-medium text-white/80">Telemetry Logging</span>
                         <input type="checkbox" defaultChecked className="accent-[#00F2FE]" />
                       </div>
                       <div className="flex justify-between items-center p-2 rounded-xl hover:bg-white/5 cursor-pointer" onClick={() => toast.success('Haptics updated', {id:'st2'})}>
                         <span className="text-[12px] font-medium text-white/80">Haptic Feedback</span>
                         <input type="checkbox" defaultChecked className="accent-[#00F2FE]" />
                       </div>
                       <div className="flex justify-between items-center p-2 rounded-xl hover:bg-white/5 cursor-pointer" onClick={() => toast.success('Anonymous Mode enabled', {id:'st3'})}>
                         <span className="text-[12px] font-medium text-white/80">Anonymous Mode</span>
                    ...
                    <div className="p-2 border-t border-white/5">
                      <Link to="/profile" onClick={() => setOpenDropdown(null)}>
                        <div className="flex items-center justify-between p-2 rounded-xl hover:bg-white/5 cursor-pointer text-[#64748B] hover:text-white transition-colors">
                          <span className="text-[11px] font-bold">Advanced Settings</span>
            ...
            >
              ...
              >
                <div className="p-4 border-b border-white/10 flex items-center gap-3">
                  ...
                  />
                  <kbd className="text-xs text-[#64748B] bg-white/5 border border-white/10 px-2 py-1 rounded-md">ESC</kbd>
                </div>
                <div className="p-2 max-h-[60vh] overflow-y-auto">
                  {searchResults.length > 0 ? (
                    searchResults.map((res, i) => (
                      <Link to={res.p} key={i} onClick={() => { setSearchOpen(false); setSearchQuery(''); }}>
                        <div className="flex items-center justify-between p-4 rounded-xl hover:bg-white/5 cursor-pointer text-white/70 hover:text-white transition-colors group">
                          <span className="font-semibold text-sm drop-shadow-md group-hover:text-[#00F2FE] transition-colors">{res.n}</span>
                          <ChevronDown className="rotate-[270deg] opacity-0 group-hover:opacity-100 transition-opacity" size={16} />
                        </div>
                      </Link>
                    ))
                  ) : (
