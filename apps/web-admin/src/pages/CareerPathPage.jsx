...
const skills = [
  ...
  { name: 'Carbon Trading',        level: 0, max: 10, xp: '0',     color: '#00F2FE', desc: 'Eco credit exchange & policy' },
  { name: 'Eco-Policy Design',     level: 0, max: 10, xp: '0',     color: '#0891B2', desc: 'Governance, SDGs, reporting' },
];
  ...
  return (
    <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-5">
      ...
      <motion.div variants={fadeUp} className="relative overflow-hidden rounded-2xl p-7"
        style={{ background: 'linear-gradient(135deg, rgba(0, 242, 254, 0.12) 0%, rgba(8, 145, 178, 0.05) 100%)', border: '1px solid rgba(0, 242, 254, 0.22)' }}>
        <div className="absolute -top-20 -right-20 w-56 h-56 rounded-full opacity-10 pointer-events-none" style={{ background: 'radial-gradient(circle, #00F2FE, transparent 70%)' }} />
        <div className="relative flex flex-col md:flex-row md:items-center gap-6">
          <div className="flex-1">
            <p className="text-[11px] font-semibold uppercase tracking-[0.15em] mb-1" style={{ color: '#22D3EE' }}>Skill Tree</p>
          ...
          <div className="grid grid-cols-3 gap-3 shrink-0">
            {[{ k: 'Total Level', v: totalLevel, c: '#00F2FE' }, { k: 'Max Possible', v: maxTotalLevel, c: '#94A3B8' }, { k: 'Completion', v: `${Math.round((totalLevel / maxTotalLevel) * 100)}%`, c: '#10B981' }].map((s, i) => (
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {skills.map((s, i) => (
          ...
          >
            ...
            <h3 className="text-[14px] font-bold text-white mb-0.5 group-hover:text-[#22D3EE] transition-colors">{s.name}</h3>
