        ...
        <div className="surface-glass p-10 rounded-[40px] border border-white/10 shadow-3xl relative overflow-hidden">
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="space-y-2">
              <label className="text-[10px] font-bold text-white/40 uppercase tracking-[0.2em] ml-1">Universal ID</label>
              <input 
                type="email" 
                value={email} 
            ...
            >
              {loading ? <Loader2 className="w-5 h-5 animate-spin" /> : (
                <span className="flex items-center gap-2">LOG IN AS ADMIN(DEMO USER) <ArrowRight size={16} /></span>
              )}
            </button>
          </form>
          ...
          <div className="relative my-8">
            ...
            <div className="relative flex justify-center text-[10px] uppercase font-bold tracking-widest"><span className="bg-[#0D1117] px-4 text-white/20">OR SECURE AUTH</span></div>
          </div>

          <div className="space-y-3">
            <button onClick={handleGoogleLogin} className="w-full py-4 bg-white/5 border border-white/10 hover:bg-white/10 hover:border-white/20 text-white/80 text-xs font-bold rounded-2xl transition-all flex items-center justify-center gap-3 group">
              <Chrome size={18} className="group-hover:text-eco-teal transition-colors" /> CONTINUE WITH GOOGLE
            </button>
