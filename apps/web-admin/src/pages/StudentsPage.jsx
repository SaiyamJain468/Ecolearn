      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/40 mt-1">
            Manage and track individual student environmental performance
          </p>
        </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-white/5 bg-white/[0.02]">
                <th className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-white/40">
                  Student
                </th>
                <th
                  className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-white/40 cursor-pointer hover:text-white transition-colors"
                  onClick={() => handleSort("class_grade")}
                >
                  Class <ArrowUpDown className="inline w-3 h-3 ml-1" />
                </th>
                <th
                  className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-white/40 cursor-pointer hover:text-white transition-colors"
                  onClick={() => handleSort("eco_points_total")}
                >
                  Points <ArrowUpDown className="inline w-3 h-3 ml-1" />
                </th>
                <th
                  className="px-6 py-4 text-xs font-bold uppercase tracking-widest text-white/40 cursor-pointer hover:text-white transition-colors"
                  onClick={() => handleSort("challenges_done")}
                >
        ...
        <div className="p-4 border-t border-white/5 flex items-center justify-between bg-white/[0.01]">
          <p className="text-xs text-white/20">
            ...
            {Math.min((page + 1) * ITEMS_PER_PAGE, sortedStudents.length)} of{" "}
          ...
          <div className="flex items-center space-x-2">
            ...
            <span className="text-[10px] font-bold text-white/40 px-2">
          ...
          <div className="bg-[#0D1120] border border-white/10 w-full max-w-md rounded-3xl p-8 relative z-10 shadow-2xl">
              ...
              <div className="flex items-center gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-white/40">
                    Class {selectedStudent.class_grade} · ID: ECO-
                    {selectedStudent.id}932
                  </p>
                </div>
              </div>
              <button
                onClick={() => setSelectedStudent(null)}
                className="p-2 hover:bg-white/10 rounded-lg text-white/30 hover:text-white transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 mb-6">
              <div className="p-3.5 rounded-xl text-center bg-white/[0.03] border border-white/5">
                ...
                <p className="text-lg font-bold text-white">
                  {selectedStudent.eco_points_total.toLocaleString()}
                </p>
              ...
              <div className="p-3.5 rounded-xl text-center bg-white/[0.03] border border-white/5">
                ...
                <p className="text-lg font-bold text-white">
                  {selectedStudent.challenges_done}
                </p>
                <p className="text-[9px] text-white/30 uppercase tracking-widest">
                  Challenges
                </p>
              ...
              <div className="p-3.5 rounded-xl text-center bg-white/[0.03] border border-white/5">
                ...
                <p
                  ...
                  style={{
                    color: getRank(selectedStudent.eco_points_total).color,
                  }}
                >
                  {getRank(selectedStudent.eco_points_total).label}
                </p>
                <p className="text-[9px] text-white/30 uppercase tracking-widest">
                  Rank
                </p>
            ...
            <div className="space-y-3 mb-6">
              ...
              <div className="flex items-center justify-between p-3 rounded-xl bg-white/[0.02] border border-white/5">
                ...
                <span className="text-xs font-semibold text-eco-green">
                  {selectedStudent.challenges_done > 0
                    ? Math.round(
                        selectedStudent.eco_points_total /
                          selectedStudent.challenges_done,
                      )
                    : 0}{" "}
                  avg
            ...
            <div className="flex gap-3">
              <button
                onClick={() => {
                  toast.success(
                    `Viewing ${selectedStudent.name}'s full activity log`,
                    { icon: "📋" },
                  );
