      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-white/40 mt-1">
            Review and validate student environmental actions
          </p>
        </div>
      ...
      {filteredSubmissions.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-3xl p-12 text-center">
          ...
          <p className="text-white/40 mt-2">
            There are no {filter} submissions to review right now.
          </p>
        </div>
      ) : (
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
          {filteredSubmissions.map((sub) => (
            <div
              key={sub.id}
              className="bg-white/5 border border-white/10 rounded-3xl overflow-hidden group hover:border-white/20 transition-all duration-300 flex flex-col sm:flex-row"
            >
              ...
              <div className="flex-1 p-6 flex flex-col justify-between">
                <div>
                  <div className="flex items-start justify-between">
                    <div>
                      ...
                      <p className="text-sm text-eco-green font-medium mb-3">
                        +{sub.challenge.points_reward} Eco Points
                      </p>
                    </div>
                ...
                {sub.status === "pending" ? (
                  ...
                  <div
                    className={`mt-4 pt-4 border-t border-white/5 flex items-center ${sub.status === "approved" ? "text-eco-green" : "text-eco-coral"}`}
                  >
                    {sub.status === "approved" ? (
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="bg-[#1F120C] border border-white/10 w-full max-w-md rounded-3xl p-8 relative z-10 shadow-2xl">
            ...
            <p className="text-white/40 text-sm mb-6">
              Briefly explain why this submission is being rejected. The student
